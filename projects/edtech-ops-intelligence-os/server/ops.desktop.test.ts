...
} = require("../electron/connectorPolicy.cjs");
...
describe("EdTech desktop domain controls", () => {
  it("enforces explicit lifecycle transitions", () => {
    ...
    expect(isTransitionAllowed("anomalies", "resolved", "investigating")).toBe(
      false
    );
  });
  ...
  it("rejects incomplete or invalid operational input", () => {
    ...
    expect(
      validateInput("incidents", {
        title: "Access issue",
        type: "platform",
        severity: "medium",
        status: "new",
        description: "Learner cannot access lab.",
      })
    ).toBe(true);
  });
  ...
  it("appends immutable audit evidence without modifying preceding audit events", () => {
    const state = {
      currentUser: { id: "u-1", name: "Maya", role: "manager" },
      audit: [
        {
          id: "audit-old",
  ...
  it("seeds a demonstrable operations workspace with traceable edge cases", () => {
    ...
    expect(
      state.students.some((student: any) => student.status === "at_risk")
    ).toBe(true);
    expect(
      state.anomalies.some((anomaly: any) => anomaly.status === "investigating")
    ).toBe(true);
  ...
  it("detects overlapping instructor, room, and learner scheduling conflicts", () => {
    ...
    expect(
      findSessionConflicts(scheduled, {
        instructorId: "ins-1",
        room: "Lab B",
        studentIds: [],
  ...
  it("publishes immutable SOP versions and calculates governed health states", () => {
    ...
    const version = publishSopVersion(
      sop,
      { body: "Revised procedure", summary: "Escalation control" },
      "Maya",
    ...
    expect(
      averageQuality({ completeness: 90, accuracy: 96, freshness: 84 })
    ).toBe(90);
    expect(
      kpiStatus({ target: 90, warningThreshold: 82, actuals: [{ value: 84 }] })
    ).toBe("watch");
  });

  it("supports accountable follow-through and production operations workspace data", () => {
    const state = createSeedState();
    expect(isTransitionAllowed("actions", "assigned", "in_progress")).toBe(
      true
    );
    expect(isTransitionAllowed("actions", "completed", "in_progress")).toBe(
      false
    );
    expect(state.actions.some((action: any) => action.decisionRequired)).toBe(
      true
    );
    expect(
    ...
    expect(state.imports.some((source: any) => source.mappingTemplate)).toBe(
      true
    );
    expect(state.reports.some((report: any) => report.status === "ready")).toBe(
      true
    );
    expect(
      state.configRecords.some(
        (entry: any) => entry.category === "Threshold governance"
      )
    ).toBe(true);
    expect(
      state.support.some(
        (check: any) => check.category === "Backup and recovery"
      )
    ).toBe(true);
  });
  ...
  it("seeds activation, integration, report execution, and import remediation controls", () => {
    const state = createSeedState();
    const managerChecklist = state.onboarding.find(
      (checklist: any) => checklist.role === "manager"
    );
    expect(
      managerChecklist.items.some(
        (item: any) => item.required && !item.completed
      )
    ).toBe(true);
    expect(
      state.configRecords.some(
        (entry: any) => entry.category === "Integration readiness"
      )
    ).toBe(true);
    expect(
      state.reports.some(
        (report: any) => report.schedule && report.snapshots?.length
      )
    ).toBe(true);
    expect(
      state.reports.some((report: any) =>
        report.runHistory?.every((run: any) => run.definitionRevision)
      )
    ).toBe(true);
    expect(
      state.imports.some((source: any) =>
        source.runs?.some((run: any) => run.status === "validation_failed")
      )
    ).toBe(true);
  ...
  it("builds reproducible report snapshots from the current workspace result set", () => {
    ...
    const report = state.reports.find(
      (entry: any) => entry.id === "rep-weekly"
    );
    const snapshot = buildReportSnapshot(report, state, {
    ...
    expect(
      snapshot.resultRows.some((row: any) => row.entity === "students")
    ).toBe(true);
  });

  it("creates validation-preview and row-outcome evidence for each student import run", () => {
    const preview = buildValidationPreview([
      "name",
      "email",
      "cohort",
      "program",
    ...
    const run = buildImportRun({
      id: "run-test",
      now: "2026-08-15T10:00:00.000Z",
      columns: ["name", "cohort", "program"],
      outcomes: [
        { row: 2, status: "created", message: "Created" },
        { row: 3, status: "duplicate", message: "Duplicate" },
        { row: 4, status: "failed", message: "Missing program" },
      ],
    });
  ...
  it("seeds governed enterprise controls and applies role boundaries", () => {
    ...
    expect(
      state.integrations.some(
        (record: any) =>
          record.system === "SIS" && Array.isArray(record.syncRuns)
      )
    ).toBe(true);
    expect(
      state.automations.some(
        (record: any) => record.approvalRequired && record.status === "active"
      )
    ).toBe(true);
    expect(
      state.privacy.some(
        (record: any) => record.requestType === "retention_review"
      )
    ).toBe(true);
    expect(
      state.plans.some((record: any) => record.forecast && record.actual)
    ).toBe(true);
    expect(
      state.resilience.some(
        (record: any) => record.control === "Backup/restore readiness"
      )
    ).toBe(true);
    expect(state.releases.some((record: any) => record.rollbackPlan)).toBe(
      true
    );
    expect(
      state.reconciliations.some(
        (record: any) => record.integrationId && record.proposedResolution
      )
    ).toBe(true);
    expect(
      state.securityControls.some(
        (record: any) => record.domain && Array.isArray(record.auditExports)
      )
    ).toBe(true);
    expect(state.dashboards.some((record: any) => record.widgets?.length)).toBe(
      true
    );
    expect(
      state.explorations.some((record: any) => record.metric && record.insight)
    ).toBe(true);
    expect(state.diagnostics.some((record: any) => record.summary)).toBe(true);
    expect(
      state.serviceReviews.some((record: any) => record.agenda && record.period)
    ).toBe(true);
  ...
  it("validates enterprise records and preserves privacy and release transitions", () => {
    ...
    expect(
      validateInput("privacy", {
        title: "Export",
        requestType: "export_request",
        status: "received",
        owner: "Maya",
      })
    ).toBe(true);
    expect(() =>
      validateInput("releases", {
        name: "Desktop",
        version: "1.2.0",
        status: "draft",
      })
    ).toThrow("owner");
    expect(isTransitionAllowed("privacy", "verifying", "fulfilling")).toBe(
      true
    );
    expect(isTransitionAllowed("privacy", "closed", "fulfilling")).toBe(false);
    expect(
      isTransitionAllowed("releases", "ready_for_review", "approved")
    ).toBe(true);
  ...
  it("creates attributable integration readiness and automation execution evidence", () => {
    ...
    const incomplete = buildIntegrationReadinessCheck(
      { credentialsConfigured: "false", mappingApproved: "true" },
      { createId, now }
    );
    const ready = buildIntegrationReadinessCheck(
      { credentialsConfigured: true, mappingApproved: true },
      { createId, now }
    );
    expect(incomplete.status).toBe("attention");
    expect(ready.status).toBe("attention");
    const mapped = buildIntegrationReadinessCheck(
      {
        system: "SIS",
        credentialsConfigured: true,
        credentialRef: "keychain://SIS_SERVICE_ACCOUNT",
        offlineSafe: true,
    ...
    expect(
      validateMappingDefinition({
        mappingDefinition: { targets: ["student.name"] },
      }).missingTargets
    ).toEqual(["student.email", "student.cohort"]);
    expect(validateCredentialReference("plain-text-token").valid).toBe(false);
    const guarded = buildGuardedSyncRun(
      {
        system: "LMS",
        credentialsConfigured: true,
        credentialRef: "env://LMS_TOKEN",
        offlineSafe: true,
        syncMode: "offline_safe",
        mappingApproved: true,
        mappingDefinition: {
          targets: ["student.name", "student.email", "student.cohort"],
        },
      },
      { createId, now, executedBy: "Maya" }
    );
    ...
    const run = buildAutomationRun(
      ...
      { createId, now, executedBy: "Maya", overrideNote: "Manager review." }
    );
    ...
    const effects = buildAutomationEffects(
      {
        id: "auto-1",
        name: "Attendance routing",
        trigger: "KPI breach",
        condition: "Below target",
        approvalRequired: true,
        escalationRule: "Escalate after 24h",
      },
      { createId, now, owner: "Maya" }
    );
  ...
  it("seeds PRD canonical records with evidence, rubric versions, and review provenance", () => {
    const state = createSeedState();
    expect(state.programs.some((record: any) => record.deliveryModel)).toBe(
      true
    );
    expect(state.cohorts.some((record: any) => record.programId)).toBe(true);
    expect(
      state.attendanceRecords.some(
        (record: any) => record.studentId && record.sessionId
      )
    ).toBe(true);
    ...
    expect(
      state.transcripts.some((record: any) => record.permittedForEvaluation)
    ).toBe(true);
    expect(
      state.rubrics.some(
        (record: any) =>
          record.activeVersion && record.versions?.[0]?.criteria?.length
      )
    ).toBe(true);
    expect(
      state.evaluations.some(
        (record: any) =>
          record.transcriptId &&
          record.evidence?.every((item: any) => item.sourceId)
      )
    ).toBe(true);
    expect(
      state.datasets.some((record: any) => record.profile && record.mapping)
    ).toBe(true);
    expect(
      state.businessReviews.some(
        (record: any) =>
          record.metricSources?.length && record.canonicalMetrics?.length
      )
    ).toBe(true);
    expect(
      state.analystRuns.some(
        (record: any) => record.sourceIds?.length && record.limitations?.length
      )
    ).toBe(true);
  });

  it("enforces governed PRD workflow transitions and permissions", () => {
    expect(isTransitionAllowed("datasets", "profiled", "mapping_ready")).toBe(
      true
    );
    expect(isTransitionAllowed("datasets", "committed", "profiled")).toBe(
      false
    );
    expect(
      isTransitionAllowed("evaluations", "pending_calibration", "approved")
    ).toBe(true);
    expect(
      isTransitionAllowed("businessReviews", "awaiting_approval", "published")
    ).toBe(true);
    expect(
      validateInput("datasets", {
        name: "Roster",
        sourceType: "CSV",
        status: "draft",
        owner: "Maya",
  ...
  it("records a bounded grounded analyst safe-failure without fabricating a model response", () => {
    ...
    expect(
      run.limitations.some((item: string) => item.includes("not configured"))
    ).toBe(true);
  ...
  it("blocks hostile generic mutations of governed PRD history while allowing human approval fields", () => {
    expect(genericMutationError("analystRuns", "create")).toContain(
      "governed workflow"
    );
    expect(
      genericMutationError("analystRuns", "update", {
        recommendation: "tamper",
      })
    ).toContain("read-only");
    expect(genericMutationError("datasets", "delete")).toContain(
      "cannot be deleted"
    );
    expect(
      genericMutationError("evaluations", "update", { evidence: [] })
    ).toContain("immutable");
    expect(
      genericMutationError("evaluations", "update", {
        status: "approved",
        calibratedBy: "Maya",
      })
    ).toBeNull();
    expect(
      genericMutationError("businessReviews", "update", {
        canonicalMetrics: [],
      })
    ).toContain("immutable");
    expect(
