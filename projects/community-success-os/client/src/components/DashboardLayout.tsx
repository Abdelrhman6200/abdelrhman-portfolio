...
function NavGroup({
  ...
  return (
    <div className="nav-group-ledger">
      <span>{label}</span>
      <SidebarMenu>
        {items.map(item => (
          <SidebarMenuItem key={item.path}>
            <SidebarMenuButton
              isActive={
                location === item.path ||
                (item.path === "/students" && location.startsWith("/students/"))
              }
...
export default function DashboardLayout({
    ...
    return (
      <div className="auth-stage">
        <div className="auth-mark">
          <img src={assetLogo} alt="Community Success OS" />
        </div>
    ...
    return (
      <div className="auth-stage">
        <div className="auth-card">
          <img src={assetLogo} alt="Community Success OS" />
          <p className="eyebrow">COMMUNITY SUCCESS OS</p>
          <h1>Operate from evidence, not a spreadsheet.</h1>
          <p>Sign in to access the protected operations workspace.</p>
          <Button onClick={() => startLogin()} className="ledger-primary">
  ...
  return (
    <SidebarProvider>
      <Sidebar className="ledger-sidebar-frame" collapsible="icon">
        <SidebarHeader className="ledger-sidebar-header">
          <button
            className="brand-ledger"
            onClick={() => setLocation("/")}
            aria-label="Go to control tower"
          >
            <img src={assetLogo} alt="" />
            <span>
              <b>Community</b>
              <small>Success OS</small>
            </span>
          </button>
        ...
        <SidebarFooter className="ledger-sidebar-footer">
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
          ...
          <button className="account-ledger" onClick={logout}>
            <Avatar className="size-8">
              <AvatarFallback>
                {user.name?.slice(0, 2).toUpperCase() || "OS"}
              </AvatarFallback>
            </Avatar>
            <span>
              <b>{user.name || "Workspace user"}</b>
              <small>
                {user.role === "admin" ? "System admin" : "Operations member"}
              </small>
            </span>
          </button>
        </SidebarFooter>
      </Sidebar>
      <SidebarInset className="ledger-inset">
        <header className="ledger-topbar">
          <div className="topbar-title">
            <SidebarTrigger className="md:hidden" />
            <span className="eyebrow">OPERATIONS WORKSPACE</span>
            <strong>Eloquenta Academy</strong>
          </div>
